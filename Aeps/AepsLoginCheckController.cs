...
{
    public class AepsLoginCheckController : ApiController
    {
        // GET api/values
        ...
        [AcceptVerbs("POST")]
        [HttpPost()]
        [ActionName("AepsLoginCheck")]
        public HttpResponseMessage AepsLoginCheck(HttpRequestMessage request)
        {
           ...
           // Password = Convert.ToString(request.Headers.GetValues("ApiPassword").First());
            ...
            return AepsLoginCheck(requestString);
        }
        ...
        [AcceptVerbs("POST")]
        [HttpPost()]
        public HttpResponseMessage AepsLoginCheck(string request)
        {
            ...
            try
            {
                ...
                if ((apiKey!=null || apiKey!="") && requestIP!="")
                {
                    ...
                    AepsLoginRequest objRQ = new AepsLoginRequest();
                    AepsLoginResponse obj = new AepsLoginResponse();
                    if (objProcessor.IsJson)
                    {
                        objRQ = JsonConvert.DeserializeObject<AepsLoginRequest>(requestString);
                    }
                    else
                    {
                    ...
                    //objRQ.TXN_Type = "W";
                    Log.Error.LogLogin(Convert.ToString(CompanyId), Convert.ToString("AepsLoginCheck" + "URL : " + requestString), "AEPSLogincheckStatusrequest");
                    bool validate = ValidateApiuser(objRQ.TokenID, requestIP); 
                    if (validate)
                    {
                        obj = objProcessor.AEPSLoginCheck(objRQ);
                        obj.TXN_Type = objRQ.TXN_Type;
                    }
                    else
                    {
                    ...
                    if (objProcessor.IsJson)
                    {
                        ...
                        Log.Error.LogLogin(Convert.ToString(objRQ.CompanyID), Convert.ToString(json), "KYCAEPSModeStatusResponse");
