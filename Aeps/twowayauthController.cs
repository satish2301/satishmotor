...
{
    public class twowayauthController : ApiController
    {
        // GET api/values
        ...
        [AcceptVerbs("POST")]
        [HttpPost()]
        [ActionName("twowayauth")]
        public HttpResponseMessage twowayauth(HttpRequestMessage request)
        {
           ...
           // Password = Convert.ToString(request.Headers.GetValues("ApiPassword").First());
            ...
            return twowayauth(requestString);
        }
        ...
        [AcceptVerbs("POST")]
        [HttpPost()]
        public HttpResponseMessage twowayauth(string request)
        {
            ...
            try
            {
                ...
                {
                    Log.Error.LogLogin(Convert.ToString(CompanyId), Convert.ToString(requestString), "DMTAEPSLoginReq");
                    ...
                    objProcessor.IPAddress = requestIP;
                    ...
                    if (validate)
                    {
                        if (objRQ.APIMode == "AEPS2")
                            ...
                            obj = objProcessor.LoginMID(objRQ);
                    ...
                    if (objProcessor.IsJson)
                    {
                        ...
                        Log.Error.LogLogin(Convert.ToString(objRQ.CompanyID), Convert.ToString(json), "DMTAEPSLoginRes");
