        ...
        public HttpResponseMessage withdrawal(string request)
        {
            ...
            try
            {
                ...
                if ((apiKey!=null || apiKey!="") && requestIP!="")
                {
                    ...
                    Log.Error.LogLogin(Convert.ToString(CompanyId), Convert.ToString(requestString), "DMTAEPSREQWAP");
                   ...
                   // validate = false;
                    if (validate)
                    {
                        if (Convert.ToDouble(objRQ.Amount) > 99)
                        {
                           if( objRQ.APIMode == "AEPS2")
                                obj = objProcessor.GetbalancerWithdrealSMSDaak(objRQ);
                           else
                            obj = objProcessor.GetbalancerWithdreal(objRQ);
                        }
                        else
                        {
                            obj.errorcode = "2";
                    ...
                    if (objProcessor.IsJson)
                    {
                        ...
                        Log.Error.LogLogin(Convert.ToString(objRQ.CompanyID), Convert.ToString(json), "DMTAEPSRESWAP");
