...
{
    public class DMTMoveToBankController : ApiController
    {
        // GET api/values
        ...
        public HttpResponseMessage GetTokens(string request)
        {
            ...
            try
            {
                ...
                if ((apiKey != null || apiKey != "") && requestIP != "")
                {
                    ...
                    MovetoBankRequest objRQ = new MovetoBankRequest();
                    MovetoBankResponse obj = new MovetoBankResponse();
                        if (objProcessor.IsJson)
                        {
                            objRQ = JsonConvert.DeserializeObject<MovetoBankRequest>(requestString);
                        Log.Error.LogLogin(Convert.ToString(objRQ.CompanyID), Convert.ToString(requestString), "DMTMOVETOBANKRequest");

                    }
                    else
                        {
                             objRQ = XmlSerializers.DeserializeFromXml<MovetoBankRequest>(requestString);
                        }
                    ...
                    if (validate)
                    {
                        ...
                        checkbalance = clsRechargeProcessor.GetAgentAvalable(objRQ.Amount.ToString(), objRQ.CompanyID);

                        if(objRQ.txnmode!= "wallet")
                            objRQ.txnmode = "AEPS";
                        if (objRQ.MODE=="" || objRQ.MODE == null)
                            objRQ.MODE = "IMPS";
                        if (objRQ.txnmode.ToUpper() == "RTGS-DS")
                            objRQ.txnmode = "wallet";
                        if (checkbalance || objRQ.Action.ToLower()!= "withdraw")
                        {
                            obj = objProcessor.MoveToBank(objRQ);
                        }
                        else
                        {
                            obj.errorCode = "2";
                        ...
                        if (obj != null)
                        {
                            ...
                            Log.Error.LogLogin(Convert.ToString(objRQ.CompanyID), Convert.ToString(json), "DmtMoveToBankResponse");
