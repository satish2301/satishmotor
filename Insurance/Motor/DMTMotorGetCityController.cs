...
{
    public class DMTMotorGetCityController : ApiController
    {
        // GET api/values
        ...
        [AcceptVerbs("POST")]
        [HttpPost()]
        public HttpResponseMessage GetCitys(string request)
        {
            ...
            try
            {
                ...
                if ((apiKey != null || apiKey != "") && requestIP != "")
                {
                    ...
                    if (string.Compare(requestContentType, "application/json") == 0)
                         ...
                         CityRequest objRQ = new CityRequest();
                        CITYResponse obj = new CITYResponse();
                        if (objProcessor.IsJson)
                        {
                            objRQ = JsonConvert.DeserializeObject<CityRequest>(requestString);
                        Log.Error.LogLogin(Convert.ToString(objRQ.CompanyID), Convert.ToString(requestString), "DMTHealthCareRequest");
                    }
                        else
                        {
                             objRQ = XmlSerializers.DeserializeFromXml<CityRequest>(requestString);
                        }
                    ...
                    {
                        // objRQ.ApiPassword
                        obj = objProcessor.GetCITY(objRQ);
                        if (obj != null)
                        {
