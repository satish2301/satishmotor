...
{
    public class DMTMotorGetDocumentController : ApiController
    {
        // GET api/values
        ...
        public HttpResponseMessage GetDocument(HttpRequestMessage request)
        {
           ...
           // Password = Convert.ToString(request.Headers.GetValues("ApiPassword").First());
            ...
            return GetDocuments(requestString);
        }
        ...
        [AcceptVerbs("POST")]
        [HttpPost()]
        public HttpResponseMessage GetDocuments(string request)
        {
            ...
            try
            {
                ...
                if ((apiKey != null || apiKey != "") && requestIP != "")
                {
                    ...
                    DocumentRequest objRQ = new DocumentRequest();
                    DocumentResponse obj = new DocumentResponse();
                    if (objProcessor.IsJson)
                    {
                        objRQ = JsonConvert.DeserializeObject<DocumentRequest>(requestString);
                        Log.Error.LogLogin(Convert.ToString(objRQ.CompanyID), Convert.ToString(requestString), "DMTRTORequest");
                    }
                    else
                    {
                        objRQ = XmlSerializers.DeserializeFromXml<DocumentRequest>(requestString);
                    }
                    ...
                    {
                        // objRQ.ApiPassword
                        obj = objProcessor.GetDocumentData(objRQ);
                        if (obj != null)
                        {
        ...
        {
            public List<InsDoc> InsDoc { get; set; }
