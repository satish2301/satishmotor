...
{
    public class DMTMotorGetModelController : ApiController
    {
        // GET api/values
        ...
        [HttpPost()]
        [ActionName("GetModel")]
        public HttpResponseMessage GetModel(HttpRequestMessage request)
        {
           ...
           // Password = Convert.ToString(request.Headers.GetValues("ApiPassword").First());
            ...
            return GetModels(requestString);
        }
        ...
        [AcceptVerbs("POST")]
        [HttpPost()]
        public HttpResponseMessage GetModels(string request)
        {
            ...
            try
            {
                ...
                if ((apiKey != null || apiKey != "") && requestIP != "")
                {
                    ...
                    if (string.Compare(requestContentType, "application/json") == 0)
                        ...
                        ModelRequest objRQ = new ModelRequest();
                        ModelResponse obj = new ModelResponse();
                        if (objProcessor.IsJson)
                        {
                            objRQ = JsonConvert.DeserializeObject<ModelRequest>(requestString);
                        Log.Error.LogLogin(Convert.ToString(objRQ.CompanyID), Convert.ToString(requestString), "DMTHealthCareRequest");
                    }
                        else
                        {
                             objRQ = XmlSerializers.DeserializeFromXml<ModelRequest>(requestString);
                        }
                    ...
                    {
                        // objRQ.ApiPassword
                        obj = objProcessor.GetModel(objRQ);
                        if (obj != null)
                        {
        ...
        {
            public List<InsModel> InsModel { get; set; }
