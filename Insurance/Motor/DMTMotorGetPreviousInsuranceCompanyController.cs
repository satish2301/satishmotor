...
{
    public class DMTMotorGetPreviousInsuranceCompanyController : ApiController
    {
        // GET api/values
        ...
        [AcceptVerbs("POST")]
        [HttpPost()]
        [ActionName("GetPreviousInsuranceCompany")]
        public HttpResponseMessage GetPreviousInsuranceCompany(HttpRequestMessage request)
        {
           ...
           // Password = Convert.ToString(request.Headers.GetValues("ApiPassword").First());
            ...
            return GetPreviousInsuranceCompany(requestString);
        }
        ...
        [AcceptVerbs("POST")]
        [HttpPost()]
        public HttpResponseMessage GetPreviousInsuranceCompany(string request)
        {
            ...
            try
            {
                ...
                if ((apiKey != null || apiKey != "") && requestIP != "")
                {
                    ...
                    if (string.Compare(requestContentType, "application/json") == 0)
                       ...
                       PreviusInsuranceCompanyRequest objRQ = new PreviusInsuranceCompanyRequest();
                       PreviusInsuranceCompanyResponse obj = new PreviusInsuranceCompanyResponse();
                        if (objProcessor.IsJson)
                        {
                            objRQ = JsonConvert.DeserializeObject<PreviusInsuranceCompanyRequest>(requestString);
                        Log.Error.LogLogin(Convert.ToString(objRQ.CompanyID), Convert.ToString(requestString), "DMTHealthCareRequest");
                    }
                        else
                        {
                            objRQ = XmlSerializers.DeserializeFromXml<PreviusInsuranceCompanyRequest>(requestString);
                        }
                    ...
                    {
                        // objRQ.ApiPassword
                        obj = objProcessor.PreviusInsuranceCompany(objRQ);
                        if (obj != null)
                        {
    ...
    public class PreviusInsuranceCompanyRequest
    {
        public string CompanyID { get; set; }
        public string TokenId { get; set; }
    }
    public class PreviusInsuranceCompanyResponse
    {
        public List<PreviusInsuranceCompany> previusInsuranceCompany { get; set; }
        public string errorMsg { get; set; }
        public string errorCode { get; set; }
        public string CompanyID { get; set; }

    }
    public class PreviusInsuranceCompany
    {
        public string PreviusInsuranceCompanyName { get; set; }
        public string PreviusInsuranceCompanyCode { get; set; }
    }
}
