...
{
    public class DMTMotorGetProductController : ApiController
    {
        // GET api/values
        ...
        public HttpResponseMessage GetProduct(HttpRequestMessage request)
        {
           ...
           // Password = Convert.ToString(request.Headers.GetValues("ApiPassword").First());
            ...
            return GetProduct(requestString);
        }
        ...
        [AcceptVerbs("POST")]
        [HttpPost()]
        public HttpResponseMessage GetProduct(string request)
        {
            ...
            try
            {
                ...
                if ((apiKey != null || apiKey != "") && requestIP != "")
                {
                    ...
                    if (string.Compare(requestContentType, "application/json") == 0)
                        ...
                        ProductRequest objRQ = new ProductRequest();
                        ProductResponse obj = new ProductResponse();
                        if (objProcessor.IsJson)
                        {
                            objRQ = JsonConvert.DeserializeObject<ProductRequest>(requestString);
                        Log.Error.LogLogin(Convert.ToString(objRQ.CompanyID), Convert.ToString(requestString), "DMTHealthCareRequest");
                    }
                        else
                        {
                             objRQ = XmlSerializers.DeserializeFromXml<ProductRequest>(requestString);
                        }
                    ...
                    {
                        // objRQ.ApiPassword
                        obj = objProcessor.GetProductData(objRQ);
                        if (obj != null)
                        {
        ...
        {
            public List<InsProduct> InsProduct { get; set; }
