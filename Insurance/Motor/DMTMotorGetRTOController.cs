...
{
    public class DMTMotorGetRTOController : ApiController
    {
        // GET api/values
        ...
        [HttpPost()]
        [ActionName("GetRTO")]
        public HttpResponseMessage GetRTO(HttpRequestMessage request)
        {
        ...
        [AcceptVerbs("POST")]
        [HttpPost()]
        public HttpResponseMessage GetRTOs(string request)
        {
            ...
            try
            {
                ...
                if ((apiKey != null || apiKey != "") && requestIP != "")
                {
                    ...
                    RTORequest objRQ = new RTORequest();
                    RTOResponse obj = new RTOResponse();
                    if (objProcessor.IsJson)
                    {
                        objRQ = JsonConvert.DeserializeObject<RTORequest>(requestString);
                        Log.Error.LogLogin(Convert.ToString(objRQ.CompanyID), Convert.ToString(requestString), "DMTRTORequest");
                    }
                    else
                    {
                        objRQ = XmlSerializers.DeserializeFromXml<RTORequest>(requestString);
                    }
                    ...
                    {
                        // objRQ.ApiPassword
                        obj = objProcessor.GetRTO(objRQ);
                        if (obj != null)
                        {
                            response = JsonConvert.SerializeObject(obj);
                            string json = Convert.ToString(response);
                            Log.Error.LogLogin(Convert.ToString(objRQ.CompanyID), Convert.ToString(json), "DMTRTOResponse");
    ...
    public class RTO
    {
        public string registered_city_name { get; set; }
