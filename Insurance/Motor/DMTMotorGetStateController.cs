...
{
    public class DMTMotorGetStateController : ApiController
    {
        // GET api/values
        ...
        public HttpResponseMessage GetState(HttpRequestMessage request)
        {
           ...
           // Password = Convert.ToString(request.Headers.GetValues("ApiPassword").First());
            ...
            return GetState(requestString);
        }

        [AcceptVerbs("POST")]
        [HttpPost()]
        public HttpResponseMessage GetState(string request)
        {
            ...
            try
            {
                ...
                if ((apiKey != null || apiKey != "") && requestIP != "")
                {
                       ...
                       clsInsuranceProcessor objProcessor = new clsInsuranceProcessor();
                        ...
                        StateRequest objRQ = new StateRequest();
                        StateResponse obj = new StateResponse();
                    if (objProcessor.IsJson)
                    {
                        objRQ = JsonConvert.DeserializeObject<StateRequest>(requestString);
                        Log.Error.LogLogin(Convert.ToString(objRQ.CompanyID), Convert.ToString(requestString), "DMTStateRequest");
                    }
                    else
                    {
                        objRQ = XmlSerializers.DeserializeFromXml<StateRequest>(requestString);
                    }
                    ...
                    {
                        // objRQ.ApiPassword
                        obj = objProcessor.GetStateData(objRQ);
                        if (obj != null)
                        {
                            response = JsonConvert.SerializeObject(obj);
                            string json = Convert.ToString(response);
                            Log.Error.LogLogin(Convert.ToString(objRQ.CompanyID), Convert.ToString(json), "DMTStateResponse");
        ...
        {
            public List<InsState> state { get; set; }
