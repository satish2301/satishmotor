...
{
    public class DMTMotorGetVariantController : ApiController
    {
        // GET api/values
        ...
        [HttpPost()]
        [ActionName("GetCity")]
        public HttpResponseMessage GetVariant(HttpRequestMessage request)
        {
           ...
           // Password = Convert.ToString(request.Headers.GetValues("ApiPassword").First());
            ...
            return GetVariant(requestString);
        }
        ...
        [AcceptVerbs("POST")]
        [HttpPost()]
        public HttpResponseMessage GetVariant(string request)
        {
            ...
            try
            {
                ...
                if ((apiKey != null || apiKey != "") && requestIP != "")
                {
                    ...
                    if (string.Compare(requestContentType, "application/json") == 0)
                       ...
                       VariantRequest objRQ = new VariantRequest();
                       VariantResponse obj = new VariantResponse();
                        if (objProcessor.IsJson)
                        {
                            objRQ = JsonConvert.DeserializeObject<VariantRequest>(requestString);
                        Log.Error.LogLogin(Convert.ToString(objRQ.CompanyID), Convert.ToString(requestString), "DMTVARIENTRequest");
                    }
                        else
                        {
                             objRQ = XmlSerializers.DeserializeFromXml<VariantRequest>(requestString);
                        }
                    ...
                    {
                        // objRQ.ApiPassword
                        obj = objProcessor.GetVarient(objRQ);
                        if (obj != null)
                        {
                            response = JsonConvert.SerializeObject(obj);
                            string json = Convert.ToString(response);
                            Log.Error.LogLogin(Convert.ToString(objRQ.CompanyID), Convert.ToString(json), "DMTVARIENTResponse");
