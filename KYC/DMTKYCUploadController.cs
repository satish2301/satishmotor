...
{
    public class DMTKYCUploadController : ApiController
    {
        // GET api/values
        ...
        public HttpResponseMessage GetTokens(string request)
        {
            ...
            try
            {
                ...
                if ((apiKey != null || apiKey != "") && requestIP != "")
                {
                    ...
                    KYCUploadRequest objRQ = new KYCUploadRequest();
                    KYCUploadResponse obj = new KYCUploadResponse();
                    if (objProcessor.IsJson)
                    {
                        objRQ = JsonConvert.DeserializeObject<KYCUploadRequest>(requestString);
                    }
                    else
                    {
                        objRQ = XmlSerializers.DeserializeFromXml<KYCUploadRequest>(requestString);
                    }
                    Log.Error.LogLogin(Convert.ToString(objRQ.CompanyID), Convert.ToString(requestString), "DMTKYCUploadRequest");
                    ...
                    if (validate)
                    {
                        if ( objRQ.CompanyID != "")
                        {
                            ...
                            obj = objProcessor.InsertKYCUpload(objRQ);
                        }
                        else
                        {
                        ...
                        if (obj != null)
                        {
                            ...
                            Log.Error.LogLogin(Convert.ToString(objRQ.CompanyID), Convert.ToString(json), "DMTKYCUploadResponse");
