...
{
    public class CreateRemitterController : ApiController
    {
        // GET api/values
        ...
        public HttpResponseMessage GetCreateRemitter(string request)
        {
            ...
            try
            {
                ...
                if (request != null)
                {
                    ...
                    RemitterRegisterRequest objRQ = new RemitterRegisterRequest();
                    RemitterRegisterResponse obj = new RemitterRegisterResponse();
                    if (objProcessor.IsJson)
                    {
                        objRQ = JsonConvert.DeserializeObject<RemitterRegisterRequest>(requestString);
                        Log.Error.LogLogin(Convert.ToString(objRQ.CompanyID), Convert.ToString(requestString), "DMTCreateRemitterRequest");
                    }
                    else
                    {
                        objRQ = XmlSerializers.DeserializeFromXml<RemitterRegisterRequest>(requestString);
                    }
