...
{
    public class GetBeneAddOTPController : ApiController
    {
        // GET api/values
        ...
        #region Get Bene Add OTP 
        [AcceptVerbs("POST")]
        [HttpPost()]
        [ActionName("GetBeneAddOTP")]
        public HttpResponseMessage GetRefundOTP(HttpRequestMessage request)
        {
            ...
            return GetRefundOTP(requestString);
        }

        // public HttpResponseMessage GetHotels([FromBody]Hotel_RQ request)
        [AcceptVerbs("POST")]
        [HttpPost()]
        public HttpResponseMessage GetRefundOTP(string request)
        {
            ...
            try
            {
                ...
                if (request != null)
                {
                    ...
                    Log.Error.LogLogin(Convert.ToString(objRQ.agentCode), Convert.ToString(requestString), "DMTGetBeneAddOTPRequest");
                    ...
                    if (validate)
                    {
                        ...
                        if (objProcessor.IsJson)
                        {
                            ...
                            Log.Error.LogLogin(Convert.ToString(objRQ.agentCode), Convert.ToString(requestString), "DMTGetBeneAddOTPResponse");
