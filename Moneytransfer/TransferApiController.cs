            ...
            try
            {
                ...
                if (request != null)
                {
                    ...
                    if (validate)
                    {
                        ...
                        checkbalance= clsRechargeProcessor.GetAgentAvalable(objRQ.TransferAmount, objRQ.CompanyID);
                        if (checkbalance)
                        {
                            if (Convert.ToDecimal(objRQ.TransferAmount) < 25001 && Convert.ToDecimal(objRQ.TransferAmount) > 99)
                            {
                                
                                    if (Channel.ToLower() == "app" && Usertype != "1")
                                        obj = objProcessor.TransferAmountAgent(objRQ);
                                    else if (Usertype == "1")
                                    {
                                        ...
                                        obj = objProcessor.TransferAmount(objRQ);
                            ...
                            {
                                obj.errorCode = "2";
                                obj.errorMsg = "Ooops!! The Amount cannot be transferred more than 25 thousand! or less then 100";
                            }
                        }
                        else
                        {
