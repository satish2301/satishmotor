...
{
    public class ComissionListController : ApiController
    {
        // GET api/values      
        ...
        [AcceptVerbs("POST")]
        [HttpPost()]
        [ActionName("ComissionList")]
        public HttpResponseMessage ComissionList(HttpRequestMessage request)
        {
            ...
            return GetComissionList(requestString);
        }


        [AcceptVerbs("POST")]
        [HttpPost()]
        public HttpResponseMessage GetComissionList(string request)
        {
            ...
            try
            {
                ...
                if (request != null)
                {
                    ...
                    ComissionListReponse obj = new ComissionListReponse();
                    if (objProcessor.IsJson)
                    {
                        objRQ = JsonConvert.DeserializeObject<Request>(requestString);
                    }
                    else
                    {
                        objRQ = XmlSerializers.DeserializeFromXml<Request>(requestString);
                    }
                    bool validate = true;// ValidateApiuser(objRQ.TokenId, requestIP, CompanyId);
                    if (validate)
                    {
                        //obj = objProcessor.ComissionList(objRQ);
                        if (objProcessor.IsJson)
                        {
