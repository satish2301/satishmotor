...
{
    public class PayoutBalanceController : ApiController
    {
        // GET api/values
        ...
        public HttpResponseMessage GetTokens(string request)
        {
            ...
            try
            {
                ...
                if ((apiKey != null || apiKey != "") && requestIP != "")
                {
                    ...
                    CheckBalaceRequest objRQ = new CheckBalaceRequest();
                    CheckBalanceResponse obj = new CheckBalanceResponse();
                        if (objProcessor.IsJson)
                        {
                            objRQ = JsonConvert.DeserializeObject<CheckBalaceRequest>(requestString);
                        }
                        else
                        {
                             objRQ = XmlSerializers.DeserializeFromXml<CheckBalaceRequest>(requestString);
                        }
                    ...
                    {
                        // objRQ.ApiPassword
                        obj = objProcessor.CheckUPIBalance(objRQ);
                        if (obj != null)
                        {
